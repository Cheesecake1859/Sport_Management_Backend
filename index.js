...
app.post('/api/register', uploadProfile.single('profileImage'), async (req, res) => {
  try {
    ...
    // 1. Normalize Interests to always be an array
    let parsedInterests = [];
    if (Array.isArray(Interests)) {
      parsedInterests = Interests; // It's already an array (multiple sports)
    } else if (Interests) {
      parsedInterests = [Interests]; // It's a single string, wrap it in an array
    }
    ...
    const newUser = new User({
      ...
      Gender,
      Phone_Number,
      Interests: parsedInterests, // Use the safe array we just created
    ...
    // Check if it's ACTUALLY a MongoDB duplicate key error (code 11000)
    if (error.code === 11000) {
